...
class dpl_vdis(dplkit.role.librarian.aLibrarian):
    """ vdis data DPL framestream
    ...
    def __init__(self, instrument,*args, **kwargs):
        ...
        import VDisLibrarian as rl
        if instrument in ('mf2hsrl','mf2vdis','magvdis','tmpvdis'):            
            self.instrument='vdis'
            self.zoo=GenericTemplateRemapNetCDFZookeeper('vdis')
            self.lib=rl.VDisLibrarian('mf2hsrl','mf2vdis',['mf2vdisM1.b1'],zoo=self.zoo)
            self.instrumentbase='mf2hsrl'
        else:
            raise RuntimeError('Unknown vdis source '+instrument)


    def __repr__(self):
        return 'DPL VDis Librarian (instrument="%s")' % (self.instrument)
    ...
    def search(self, start_time_datetime=None, end_time_datetime=None,reverse_padding=None,timeres_timedelta=None,window_width_timedelta=None,forimage=None,inclusive=False,timesource=None,allow_nans=False,*args, **kwargs):
        ...
        """
        if len(args):
            print 'Unused dpl_vdis.search args = ',args
        if len(kwargs):
            print "Unused dpl_vdis.search kwargs = ",kwargs
        ...
        if timeres_timedelta!=None and nar.vdisNativeTimeResolution>timeres_timedelta:
            #dropping time resolution to 'pure native'
            timeres_timedelta=None

        if inclusive:
            #remake with padding
            padAmount=(2*timeres_timedelta) if timeres_timedelta!=None else (5*nar.vdisNativeTimeResolution)
            if d['starttime']:
