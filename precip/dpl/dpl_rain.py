...
class dpl_rain(dplkit.role.librarian.aLibrarian):
    """ Rain data DPL framestream
    ...
    def __init__(self, instrument,*args, **kwargs):
        ...
        import RainLibrarian as rl
        if instrument in ('mf2hsrl','mf2rain','magrain','tmprain'):            
            self.instrument='rain'
            self.zoo=GenericTemplateRemapNetCDFZookeeper('rain')
            self.lib=rl.RainLibrarian('mf2hsrl','mf2rain',['mf2rainM1.b1.'],zoo=self.zoo)
            self.instrumentbase='mf2hsrl'
        else:
            raise RuntimeError('Unknown rain source '+instrument)


    def __repr__(self):
        return 'DPL Rain Librarian (instrument="%s")' % (self.instrument)
    ...
    def search(self, start_time_datetime=None, end_time_datetime=None,reverse_padding=None,timeres_timedelta=None,window_width_timedelta=None,forimage=None,inclusive=False,timesource=None,allow_nans=False,*args, **kwargs):
        ...
        """
        if len(args):
            print 'Unused dpl_rain.search args = ',args
        if len(kwargs):
            print "Unused dpl_rain.search kwargs = ",kwargs
        ...
        if timeres_timedelta!=None and nar.rainNativeTimeResolution>timeres_timedelta:
            #dropping time resolution to 'pure native'
            timeres_timedelta=None

        if inclusive:
            #remake with padding
            padAmount=(2*timeres_timedelta) if timeres_timedelta!=None else (5*nar.rainNativeTimeResolution)
            if d['starttime']:
