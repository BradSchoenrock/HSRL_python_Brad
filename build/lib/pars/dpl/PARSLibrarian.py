...
@dplkit.role.decorator.exposes_attrs_in_chain(['parsType','parsNativeTimeResolution'])
class PARSNarrator(ARMNarrator.ARMNarrator):
    ...
    @property
    def parsNativeTimeResolution(self):
    ...
    def __init__(self,host,basedir,instname,dataprefix,start,end):
        super(PARSNarrator,self).__init__(basedir,instname,dataprefix,'pars',start,end)
        self.basedir=basedir
        self.instrumentname=instname
        self.timeres=None


class PARSLibrarian(dplkit.role.librarian.aLibrarian):
    ...
    def search(self,start,end,*args,**kwargs):
        ...
        """
        ret= PARSNarrator(self,self.basedir,self.instrumentname[self.instrumentname.find('pars'):],self.dataprefix,start,end)
...
def main():
    ...
    zoo=GenericTemplateRemapNetCDFZookeeper('pars')
    lib=PARSLibrarian('/data/mf2hsrldata','magpars2S1','magpars2S1.b1.',zoo=zoo)
    zoo=None
    m=lib(start=datetime(2013,6,24,20,0,0),end=datetime(2013,6,24,22,0,0))
