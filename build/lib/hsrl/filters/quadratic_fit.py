...
def quadratic_fit(bin0,bin1,bin2,v0,v1,v2,s0,s1):
    """quadratic_fit(bin_vec,bin0,bin1,v0,v1,s0,s1)
       ...
       and derivatives s0 and s2 at bin0 and bin2.
       e*bin^4 + d*bin^3 + c*bin^2 + b*bin + a are returned as a vector [e,d,c,b,a]"""

   
  
    in_array = np.array([[bin0**4,bin0**3,bin0**2,bin0,1.0]
                        ,[bin1**4,bin1**3,bin1**2,bin1,1.0]
                        ,[bin2**4,bin2**3,bin2**2,bin2,1.0] 
                        ,[4.0*bin0**3,3.0*bin0**2,2.0*bin0,1.0,0.0]
                        ,[4.0*bin2**3,3.0*bin2**2,2.0*bin2,1.0,0.0]])
    
    in_vector = np.array([v0,v1,v2,s0,s1])
