...
class dpl_vdis_images_artist(dplkit.role.artist.aArtist):
    """ Artist for rendering VDIS Images

    The busy work on this artist is all in vdis.graphics.vdis_display.show_vdis()
    ...
    def __init__(self,framestream,display_defaults,instrument=None,framedomain=None,figurecontainer=None):
        super(dpl_vdis_images_artist,self).__init__(framestream)
        ...
        self.instrument=instrument if instrument!=None else 'VDis'
        self.framestream=framestream
        self.framedomain=framedomain
        import precip.graphics.vdis_display as rd
        self.rd=rd
    ...
    def renderframe(self,frame):
            print 'vdis render frame called'
            myframe=frame
            allc=False
            ...
            if self.framedomain==None:
                ...
                if usetimes==None or usealts==None:
                    print "Can't find native vdis axes in artist."
            if self.framedomain!=None and hasattr(rs,self.framedomain):
                ...
                if usetimes==None and hasattr(nativeframe,'times'):
                    usetimes=nativeframe.times
                if usetimes==None or usealts==None:
                    print "Can't find native vdis range axes in artist."
    ...
    def rendernativeframe(self,rs,usetimes,usealts):
            ...
            self.rd.show_vdis(instrument,display_defaults,rs,usetimes,usealts,figs)
