...
class dpl_pars_images_artist(dplkit.role.artist.aArtist):
    """ Artist for rendering PARS Images

    The busy work on this artist is all in pars.graphics.pars_display.show_pars()
    ...
    def __init__(self,framestream,display_defaults,instrument=None,framedomain=None,figurecontainer=None):
        super(dpl_pars_images_artist,self).__init__(framestream)
        ...
        self.instrument=instrument if instrument!=None else 'PARS'
        self.framestream=framestream
        self.framedomain=framedomain
        import pars.graphics.pars_display as rd
        self.rd=rd
    ...
    def renderframe(self,frame):
            print 'pars render frame called'
            myframe=frame
            allc=False
            ...
            if self.framedomain==None:
                ...
                if usetimes==None or usealts==None:
                    print "Can't find native pars axes in artist."
            if self.framedomain is not None:
                if not hasattr(rs,self.framedomain):
                    print 'Subframe ',self.framedomain,'is missing!!!'
                    return
                ...
                if usetimes==None and hasattr(nativeframe,'times'):
                    usetimes=nativeframe.times
                if usetimes==None or usealts==None:
                    print "Can't find native pars range axes in artist."
    ...
    def rendernativeframe(self,rs,usetimes,usealts):
            ...
            self.rd.show_pars(instrument,display_defaults,rs,usetimes,usealts,figs)
